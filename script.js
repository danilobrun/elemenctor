 ...
 */
const cartSidebar =
  document.querySelector(".cart-sidebar"); /*Seleciona .cart-sidebar*/
function openSidebar(event) {
  //Abrir carrinho
  event.stopPropagation(); //Para de propagar o evento para os elementos pai
  cartSidebar.classList.add(
    "cart-sidebar-open"
  ); /*na lista de classes do cartSidebar add uma nova classe definida em nosso CSS*/
}
function closeSidebar() {
  cartSidebar.classList.remove(
    "cart-sidebar-open"
  ); /*removemos a classe que faz o cart abrir*/
}
const btnCart =
  document.querySelector("#btn-cart"); /*Selecionando o elemento sacola*/
btnCart.addEventListener(
  "click",
  openSidebar
); /*(evento, ação) openSidebar nesse caso é una função de callBack*/
const btnCloseCart =
  document.querySelector("#btn-close-cart"); /*Elemento fechar sacola*/
btnCloseCart.addEventListener(
  "click",
  closeSidebar
); /*evento click, fechar carrinho função de callBack*/
const addMore = document.querySelector("#add-more"); /*Elemento btn-outline*/
addMore.addEventListener(
  "click",
  closeSidebar
); /*evenco click, fechar sidebar*/
  ...
  fetch("/products.json") //url, {} array de config
    ...
    .then((body) => {
      ...
      body.groups.forEach((group) => {
        //agora recebe um parâmetro (que guardará nosso obj JSON)
        ...
        group.products.forEach((product) => {
          //acessando array de produtos e lendo cada item do array
          const description =
            product.description != null ? `<p>${product.description}</p>` : ""; //Operador ternario
          groupHtml += `<article class="card">
            ...
            <div class="card-content">
                ...
                >Comprar</button>
                <a class="btn-catalogo btn-catalogo btn-block btn-add-cart" href="${
                  product.catalogo
                }" target="_blank">Acesse o Catálogo</a>
            </div>
        </article>`;
        });
...
const addToCart = (event) => {
  //função para adicionar no carrinho, checa se o produto já esta no carrinho caso não add e + 1 em qty
...
function removeOfCart() {
  //Remoção cria uma copia do array mesmo aquele ID que queremos excluir
...
const handleKeydown = (event) => {
  //função para impedir caracteres indevidos
  if (event.key == "-" || event.key == ".") {
    //seleciona event.key é o valor
...
const setupCartEvents = () => {
  ...
  btnRemoveCartEls.forEach((btn) => {
    //Itera sobre o array de inputs com parâmetro btn para usar depois na função
  ...
  inputsQtyEl.forEach((input) => {
    //Itera o array e executa uma função passando um parâmetro input
...
const handleCartUpdate = (renderItens = true) => {
  //Rederização do carrinho por padrão é true ou seja na hora de chamar ele não preciso dizer se ele é true
  ...
  if (totalCart > 0) {
    //Lógica caso totalCart > 0 (tem algum item no carro) mostre os produtos
    ...
    if (renderItens) {
      ...
      productsCart.forEach((product) => {
        //iterar sobre o array, abaixo add ele mesmo string vazia e depois recebe a template string
        cartItensParent.innerHTML += `<li class="cart-item">
          ...
          product.name
                ...
                <div>
                    ...
                    <p class="price">R$ ${product.price.toLocaleString(
                      "pt-br",
                      { minimumFractionDigits: 2 }
                    )}</p>
                </div>
                <input class="form-input input-qty-cart" type="number" min="0" value="${
                  product.qty
                }" data-id="${product.id}">
    ...
    const totalPrice = productsCart.reduce((total, item) => {
      //somar o valor total R$
      return total + item.qty * item.price;
    }, 0);
    cartTotalValueEl.innerText =
      "R$ " + totalPrice.toLocaleString("pt-br", { minimumFractionDigits: 2 });
  } else {
    //Caso não mostre o carrinho vázio
...
const initCart = () => {
  ...
  if (savedProducts) {
    //se savedProducts for true ( != null )
...
const handleCheckoutSubmit = (event) => {
  /*função simples para interromper o evento do submit*/
  ...
  const complement = event.target.elements["input-complement"].value
    ? `- ${event.target.elements["input-complement"].value}`
    : "";
  ...
  window.open(
    `https://${subdomain}.whatsapp.com/send?phone=5581979139104&text=${text}`,
    "_blank"
  );
};
const formCheckoutEl =
  document.querySelector(".form-checkout"); /*Captura El formulário*/
formCheckoutEl?.addEventListener(
  "submit",
  handleCheckoutSubmit
); /*Adiciona uma esconta ao EL do tipo submit e executa a função interromper o submit*/

/* Masks */
const inputPhoneEl = document.querySelector("#input-phone");
IMask(inputPhoneEl, {
  /*Executa a função IMask(como parâmetro passamos nosso elemento) */
